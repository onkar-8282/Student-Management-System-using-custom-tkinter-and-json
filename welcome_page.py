...
def create_welcome_page(root, show_student_login, show_admin_login, show_sign_up):
    ...
    student_login_btn = tk.Button(
        frame, text="Student Login", font=('bold', 15),
        bd=0, bg=bg_color, fg='white', command=show_student_login
    )
    student_login_btn.place(x=520, y=125, width=260)

    student_login_img = tk.Button(
        frame, image=student_login_icon, bd=0,
        command=show_student_login
    )
    student_login_img.place(x=420, y=100)

    # Admin Login
    admin_login_btn = tk.Button(
        frame, text="Admin Login", font=('bold', 15),
        bd=0, bg=bg_color, fg='white', command=show_admin_login
    )
    admin_login_btn.place(x=520, y=250, width=260)

    admin_login_img = tk.Button(
        frame, image=admin_login_icon, bd=0,
        command=show_admin_login
    )
    admin_login_img.place(x=420, y=225)

    # Sign Up
    add_student_btn = tk.Button(
        frame, text="Sign Up", font=('bold', 15),
        bd=0, bg=bg_color, fg='white', command=show_sign_up
    )
    add_student_btn.place(x=520, y=375, width=260)

    add_student_img = tk.Button(
        frame, image=add_student_icon, bd=0,
        command=show_sign_up
    )
    add_student_img.place(x=420, y=350)
